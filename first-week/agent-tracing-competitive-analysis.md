...
- **[建议] 验证靶子选择：** 差异化验证应优先选择**没有低成本工程解法**的故障类别，具体为 FM-07 检索排序漂移、FM-08 多轮上下文冲突、FM-09 重试状态放大三类（编号、案例与证据以 `second-week/docs/00-故障分类与案例事实源.md` 为准）。理由：这三类故障执行层全部成功（无异常、无超时、HTTP 全 200），规则引擎判不出任何问题，必须靠因果图 + 语义分析 + 跨步骤证据引用才能定位——只有它们能证明证据化诊断的必要性；而状态过期一类存在 TTL + 变更订阅的低成本解法，用它做主验证场景撑不起价值主张，仅作为「规则可判定类」的次要对照类别保留。
...
**[建议]** 3 类典型故障明确为 FM-07 检索排序漂移、FM-08 多轮上下文冲突、FM-09 重试状态放大（编号与案例事实以 `second-week/docs/00-故障分类与案例事实源.md` 为准）——三类均无低成本工程解法、执行层全部成功、规则判不出，最能验证证据化根因定位的必要性；状态过期（FM-01）作为规则可判定的次要对照类别保留，不再作为首要验证场景。
...
| 第 4 天 | 诊断 v0 | 规则/不变量兜底确定性故障；因果图 + Diff + 模型语义覆盖 FM-07/08/09 三类，输出候选根因、证据和置信度 |
