...
> 交付日期：2026-08-17
> 配套原型：`second-week/agent-tracing-prototype.html`（单文件、离线可开、可点击，版本 v1.4，已并入可用性走查迭代与微交互动效）
...
**产品一句话定义：** CauseLens（因镜）为运行单 Agent 系统（一个 Agent 多步骤执行）的平台团队与 SRE（站点可靠性工程师），在生产事故发生时，给出**最早的故障因（首故障点）、完整证据链和验证办法**，而不是让人在几百个 Span（链路片段）里自己找答案。
...
├───────────────────────────────────────────────────────┤
│ [筛选行] 簇 ▾  步骤 ▾  时间 ▾  裁决状态 ▾  置信度 ▾        │
├───────────────────────────────────────────────────────┤
│ [事故表] ID │ 时间 │ 任务类型 │ 症状 │ 首故障点(候选) │ 置信度条 │ 裁决状态 │
...
> 为【运行单 Agent 生产系统的平台团队与 SRE】在【生产事故排查】情境下解决【报错步骤不等于根因步骤、证据散落难以定位】的问题，通过【首故障点定位 + 证据化诊断 + 成功/失败 Diff + 裁决闭环】实现【MTTR 下降、同类问题不复发、修复可验证】。
...
- **功能描述：** 失败 Trace 自动归入失败模式簇（第一期规则聚类：按首故障点类型 + 报错工具 + 错误码组合；**[决策]** 语义聚类放第二期，见任务 12 决策 D-4）。簇卡片展示：模式名称、24h 发生次数、影响会话数、趋势、最高置信度诊断摘要。列表支持按步骤类型、时间段、裁决状态、置信度筛选。
...
原型文件：`second-week/agent-tracing-prototype.html`。单文件、内嵌 CSS/JS、离线双击可开。当前版本 **v1.4**：在 v1.1（走查迭代 + 微交互）基础上将配色重构为主流平台风格（浅灰白底 + 白色卡片 + 品牌蓝），案卷隐喻收敛为衬线标题、裁决印章、荧光笔标记三个签名元素（见 11.1 节视觉体系）。
...
| 编号 | 画像 | 背景设定 | 关注点 |
|---|---|---|---|
| A | Agent 开发者 | 维护「订单助手」Agent 的退款执行步骤，日常改 prompt 和工具编排 | 证据能不能核实、裁决是否可靠 |
...
设计目标只有一个：**让用户信任诊断结论**。所有动效服务于「确定性感」与「克制感」，不做装饰性动画。全部动效已在原型 v1.4 中用 CSS/JS 实现（实现位置见下表），时长 150–400ms（例外：模拟异步分析的加载态，以及证据定位闪烁 1.6s、新用例行背景闪淡 1.2s 两处驻留提示），不阻塞任何操作，并通过 `prefers-reduced-motion` 尊重系统减弱动效设置。
...
- **视觉方向「主流平台底盘 + 案卷签名元素」（v1.4）：** 界面采用主流工程平台的浅色配色——浅灰白底 `#f6f8fa`、白色卡片、石板灰正文，品牌蓝 `#2563eb` 统一承担框架与可执行动作（主按钮、链接、导航激活项）。产品的庭审/证据隐喻不再铺满整个界面，收敛为三个「签名元素」：①标题用衬线字体（Georgia + 宋体回退，传达公文感），数据用等宽字体；②裁决印章（双线框 + 微倾，见 11.2 节点 2）；③证据定位高亮是荧光笔标记（黄底 + 墨色竖条），像在案卷上划重点。**品牌蓝只表达「可操作」，不表达判断；语义色一旦出现即有含义**——与「无证据的结论不上屏」同构。
- **色彩语义（与证据体系绑定，全站一致）：**

| 色 | 语义 | 使用处 |
|---|---|---|
| 品牌蓝 `#2563eb` | 框架 / 可执行动作（不表达语义，只表达「可操作」） | 主按钮、链接、导航激活项、候选排名 |
| 印章红 `#dc2626` | 裁决已盖章生效（与错误红同色值，靠「双线框印章」形态区分） | 裁决「成立」印章、键盘焦点框 |
| 规则青 `#0d9488` | 确定性 / 规则判定 / 支持证据 | 规则标签、支持证据左边条 |
| 成功绿 `#16a34a` | 裁决成立 / 通过状态 | 成立按钮、通过标签、裁决落定条 |
| 警示琥珀 `#b45309` | 反证 / 警告 / 待处理 / 首故障点 | 辩方、门禁警告、⚑ 首故障点 |
| 错误红 `#dc2626` | 报错事实（仅陈述事实，不渲染情绪） | 报错点、错误码、失败链表头 |
| 模型紫 `#7c3aed` | 模型推断（提示不确定性高于规则青） | 模型标签、语义分析加载态 |
| 荧光标记黄 `#fef08a` | 定位 / 划重点 | 证据跳转的驻留高亮 |
