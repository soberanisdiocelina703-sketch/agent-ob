...
> 交付日期：2026-08-17
> 配套原型：`second-week/agent-tracing-prototype.html`（单文件、离线可开、可点击，版本 v1.6，已并入可用性走查迭代与微交互动效）
...
**产品一句话定义：** 寻迹为运行单 Agent 系统（一个 Agent 多步骤执行）的平台团队与 SRE（站点可靠性工程师），在生产事故发生时，给出**最早的故障因（首故障点）、完整证据链和验证办法**，而不是让人在几百个 Span（链路片段）里自己找答案。

**核心流程一张图：**

```mermaid
flowchart LR
  A[Agent运行数据] --> R[运行记录:成功/失败 Trace]
  R -->|失败且触发事故| B[事故列表:失败模式聚类]
  R -->|成功| N[正常 Trace 详情/对比基线]
...
3. **三步闭环替代跨模块跳转** —— 从发现问题到生成回归用例，主操作不超过三次（打开事故 → 复核 → 转用例）。

**基础运行可见性：** 成功与失败 Trace 使用同一运行记录入口；正常 Trace 可查看完整步骤并选作 Diff 基线，但不会触发事故诊断。

**设计增强：** 「双向证据复核」——在标准诊断详情页中并列呈现支持证据、反面证据与其他解释，由用户核对原始记录后确认。创新落在证据组织与反馈质量上，不增加角色隐喻和操作仪式。

**最大风险：** 诊断误报导致信任崩塌。缓解：确定性规则先行、证据强制引用、低置信度降级为「线索」而非「结论」、复核反馈闭环校准（详见任务 13）。
...
14. [任务 14：设计增强：双向证据复核](#任务-14设计增强双向证据复核)
...
**[承接]** 第一周选定场景「跨步骤因果故障定位与证据化回放」（第一周 4.1 节，原「跨 Agent」表述，本周叙述聚焦调整为单 Agent 内的多步骤执行），主流程覆盖从问题触发到复发预防的完整闭环。
...
**[细化]** 与第一周 2.4 节 AgentLoop 信息架构的关键差异：AgentLoop 以「数据类型」组织导航（链路 / 会话 / 模型 / 工具分析），寻迹保留主流的运行记录入口，并以「故障假设的推进」组织诊断导航（发现 → 诊断 → 验证 → 沉淀），每个页面回答排障过程中的一个问题。

---

## 任务 2：Demo 输出（页面设计稿）

本章是任务 7 可点击原型的蓝图：对站点地图（1.3 节）中的 8 个主页面及正常 Trace 详情逐一给出设计稿。每份设计稿包含页面目标、区块划分（ASCII 布局）、**第一眼要素**（打开页面 3 秒内必须看到的结论性信息，落实设计原则 1「结论先行」）、主/次操作与页面间数据传递。原型页面与本章一一对应（对照关系见 7.1 节）。
...
├────────────────────────────────────────────────────────┤
│ [筛选] 状态：全部/成功/失败 │ 任务 │ Agent版本 │ 时间      │
├────────────────────────────────────────────────────────┤
│ Trace ID │ 状态 │ 时间 │ 任务 │ 版本 │ 步骤 │ 耗时 │ 关联信息 │
└────────────────────────────────────────────────────────┘
                  ↓ 点击成功 Trace
┌────────────────────────────────────────────────────────┐
│ [成功状态 + Trace摘要]                  [设为对比基线]    │
...
├───────────────────────────────────────────────────────┤
│ [筛选行] 簇 ▾  步骤 ▾  时间 ▾  复核状态 ▾  置信度 ▾        │
├───────────────────────────────────────────────────────┤
│ [事故表] ID │ 时间 │ 任务类型 │ 症状 │ 首故障点(候选) │ 置信度条 │ 复核状态 │
│ [表尾] 历史案例提示：已确认根因 N 条 → 历史案例库用于本簇置信度加权   │
└───────────────────────────────────────────────────────┘
```
...
│  #1 记忆快照过期  [规则判定] 86% ─ 展开=证据抽屉(支持4/反证2)+复核入口 │
│  #2 契约缺字段    [模型推断] 41% ─ 含强反证提示                      │
│  #3 线索(折叠)    [模型推断] 22% ─ 低置信降级，复核入口禁用(G-2)      │
├───────────────────────────────────────────────────────┤
...
- **数据传递（→ 复核）：** 用户在 Diff 页确认过的差异不改变数据，但 Diff 证据（E2/E4）在复核页的支持证据中被引用回跳。
...
│  1️⃣过期事实(铁证·规则)      │  ⚡C1 过期不必然失败 + 分析说明        │
│  2️⃣数据失效  3️⃣传播链完整   │  ⚡C2 替代解释:限流 + 分析说明         │
│  4️⃣反事实成立 + 历史案例引用    │  ℹ️ 反面证据声明:未找到其他有效反证        │
├───────────────────────────┴───────────────────────────┤
...
**[承接第一周 4.2 节]** 运行单 Agent 系统的团队（AI 平台负责人、Agent Owner、SRE）面临一个具体困境：**最后报错的步骤不是根因步骤**。Agent 在前面的步骤写入了过期状态、传参时丢了字段、记忆里存了脏数据，错误沿着执行链放大，最后在某个下游工具调用处爆掉。现有工具（含 AgentLoop）能把几百个 Span 完整展示出来——树、图、时序、轨迹五种视图——但「哪一步先出的错、为什么」仍然要人翻着看。

代价可量化：排障工时（一次跨步骤事故平均需要资深工程师半天到两天）、事故影响时间（MTTR，平均修复时长）、同类问题复发（经验沉在个人聊天记录里，换个人接手重新踩坑）。

### 3.2 解决什么问题

**场景定义（一句话，承接第一周 4.7 节格式）：**

> 为【运行单 Agent 生产系统的平台团队与 SRE】在【生产事故排查】情境下解决【报错步骤不等于根因步骤、证据散落难以定位】的问题，通过【首故障点定位 + 证据化诊断 + 成功/失败 Diff + 复核闭环】实现【MTTR 下降、同类问题不复发、修复可验证】。
...
- **用户故事：** 作为 Agent 开发者，我想查看成功和失败的全部运行，确认采集是否正常，并能打开一次正常执行核对步骤或选作对比基线。
- **功能描述：** 运行记录按时间倒序展示成功与失败 Trace，支持状态、任务、Agent 版本、时间和 Trace ID 筛选。成功 Trace 详情展示输入输出、运行摘要、完整 Trace 树与步骤耗时；失败 Trace 直接关联事故。正常 Trace 可手动设为成功/失败 Diff 基线。
- **输入/输出：** 输入项目 ID 与筛选条件；输出 Trace 摘要列表或单条 Trace 详情。
- **边界：** 不扩展为通用 APM 指标分析；正常 Trace 不运行根因诊断、不产生事故、不进入复核流程；大 Trace 继续按需加载。
- **验收标准：** AC2A-1 全部/成功/失败筛选结果准确；AC2A-2 正常 Trace 详情可查看完整步骤且不出现诊断区；AC2A-3 从正常 Trace 选择基线后 Diff 页基线 ID 同步更新；AC2A-4 万条 Trace 下列表首屏 ≤ 2s。

### M2 事故列表与失败模式聚类（P0）

- **用户故事：** 作为 SRE，值班时我想知道「今天最该先处理哪类失败」，而不是逐条翻错误 Trace。
- **功能描述：** 失败 Trace 自动归入失败模式簇（第一期规则聚类：按首故障点类型 + 报错工具 + 错误码组合；**[决策]** 语义聚类放第二期，见任务 12 决策 D-4）。簇卡片展示：模式名称、24h 发生次数、影响会话数、趋势、最高置信度诊断摘要。列表支持按步骤类型、时间段、复核状态、置信度筛选。
...
- **用户故事：** 作为开发，我想把这条失败执行和一条正常执行摆在一起，看看状态、传参、工具参数差在哪。
- **功能描述：** 系统按「同 Agent 版本 + 同任务类型 + 时间邻近」自动推荐基线成功链（用户可手动更换）。三个维度并排对比：状态 Diff（state_before/after 的字段级差异）、传参 Diff（步骤间契约字段有无与值差异）、工具 Diff（参数与返回的差异）。差异行高亮，与候选根因相关的差异行标记「证据」角标。
...
- **边界：** 复核不可跳过——未复核的事故不能转回归用例（**[决策]** 见任务 12 决策 D-3）；复核权限仅限项目成员中的「复核人」角色。
...
- **功能描述：** 「成立」复核后一键转回归用例：自动打包输入、上下文快照（脱敏后）、期望不变量（从根因反推，如「refund_amount ≤ refundable_amount」）。用例归入回归集。新版本发布前运行回归集，输出门禁报告：通过 / 警告 / 阻断。**[决策]** 默认只警告不阻断，团队可按回归集配置为阻断（依据见任务 12 决策 D-5）。
...
- **功能描述：** 项目级隔离；角色三档：查看者（只读）、分析师（可诊断、不可复核）、复核人（可复核与重新复核）；脱敏策略（字段白名单 + 正则规则，采集端优先执行，**[承接]** 第一周 11.4 节）；告警规则（按失败模式簇的次数/比例阈值）。
...
| G-2 置信度过低 | 所有候选置信度 < 40% | 候选降级为「线索」，折叠展示，不进入复核流程 | 「以下为低置信度线索，仅供参考，不建议直接采信」 | 补充数据后可触发重新诊断 |
...
| G-5 无可比基线 | 「同版本 + 同任务 + 时间邻近」三条件检索不到成功链 | Diff 页明示无基线并允许跳过，不硬凑相似度不足的基线误导用户 | 「未找到满足条件的可比成功链。你可以放宽条件手动选择，或跳过 Diff 直接进入复核」 | 放宽检索条件手动选基线，或跳过 |
...
原型文件：`second-week/agent-tracing-prototype.html`。单文件、内嵌 CSS/JS、离线双击可开。当前版本 **v1.6**：采用主流工程后台的浅灰背景、白色卡片、左侧导航、无衬线字体与品牌蓝主操作；设计增强集中在首故障点、证据回跳和双向证据复核（见 11.1 节）。

**演示用模拟案例（与本文档同一套数据）：** 「订单助手」单 Agent 客服系统——一个 Agent 按五个步骤执行（规划、订单查询、知识/记忆检索、退款执行、通知）。核心事故 `INC-2026-0814-003`：用户申请退款 ¥180，退款执行步骤调用退款接口返回 422 失败。**表面报错**在第 9 步退款工具调用；**真实根因**在第 3 步——知识检索步骤读取了 4 小时前的记忆快照 `order_snapshot v3`（状态 PAID、可退 ¥180），而订单真实状态已是 PARTIALLY_REFUNDED（可退 ¥60），过期状态经步骤间传参传给退款执行步骤，导致超额退款请求。
...
**异常状态演示（对应任务 6）：** ①体检页断链警告（D-1，含修复指引）；②工作台候选 3 为低置信度「线索」折叠态、复核入口禁用（G-2）。另有 U-3 权限禁用态（设置页「邀请成员」）与快照过期提示（门禁页 REG-1015）。
...
### 8.2 核心接口草案（10 个关键 API）

| # | 方法与路径 | 用途 | 请求要点 | 响应要点 |
|---|---|---|---|---|
| 1 | `POST /v1/otlp/traces` | OTLP 标准接入 | OTLP protobuf/JSON；扩展属性走 `xunji.*` 命名空间（task/handoff/state/memory/artifact 事件） | 202 + 采集批次 ID |
| 2 | `GET /v1/projects/{pid}/checkup` | 采集体检报告 | 时间范围 | 四项得分、问题清单（含 span 定位）、修复指引 key |
| 2A | `GET /v1/projects/{pid}/traces` | 运行记录 | 筛选：status、task、agent_version、time、trace_id；分页游标 | Trace 摘要列表 + 成功率/P95 等聚合摘要 |
| 2B | `GET /v1/traces/{tid}` | Trace 详情 | 可选 include=spans,io,metrics | 状态、输入输出摘要、运行指标、完整 spans[]；关联 incident_id? |
...
| 6 | `POST /v1/candidates/{cid}/review` | 提交复核 | result: confirmed/excluded/insufficient；reason_code；correct_cause_ref?；If-Match 版本头（并发控制，对应异常 U-2） | 复核记录；409 时返回已有复核供附议/重新复核 |
...
spans                                      -- 原子步骤宽表
  ...
  step_name String, agent_version String, status Enum(OK|ERROR), error_code String
...
events_state                               -- 状态读写事件
  ...
  writer_step/reader_step String, snapshot_version String, snapshot_age_s UInt32

events_handoff                             -- 步骤间传参事件
  source_step/target_step String, contract_version String
...
verdicts(id, candidate_id, verdict ENUM(upheld, rejected, insufficient),
          ...
          superseded_by NULLABLE)                     -- 重新复核链（对应异常 U-1）
...
| TC-16 | G-2 | 所有候选置信度 < 40% | 打开诊断 | 候选折叠为「线索」，复核入口禁用 | P1 |
...
| TQ-03 | 置信度校准 | 按置信度分桶统计根因确认率 | 高置信桶（>80%）确认率显著高于低置信桶；期望校准误差 ECE ≤ 0.15 | P1 |
...
| 编号 | 画像 | 背景设定 | 关注点 |
|---|---|---|---|
| A | Agent 开发者 | 维护「订单助手」Agent 的退款执行步骤，日常改 prompt 和工具编排 | 证据能不能核实、复核是否可靠 |
...
| T3 | 对候选 1 完成确认是根因，转成回归用例并在门禁页确认 | 走通复核→弹窗→门禁三步 | 主操作次数（目标 ≤ 3）、卡点位置 |
| T4 | 判断本项目采集数据是否可信，断链在哪、怎么修 | 到体检页说出断链位置与修复方向 | 完成与否、是否发现修复指引 |
| T5 | 解释候选 3 为什么不能进复核 | 说出「低置信度线索、不建议采信」 | 是否理解禁用原因、是否误点 |
...
| T3 | ✔ 完成 | 3 次主操作走通。反馈：「复核按钮没有二次确认，怕手滑点了成立」（→ 问题 P-5） |
| T4 | ✔ 完成 | 好评修复指引里有可复制代码 |
...
| T2 | ✔ 完成 | — |
| T3 | ✔ 完成 | 看到置信度 86% 后表示「基本就直接点成立了，说实话没细看反面证据」（→ 问题 P-4） |
| T4 | ✔ 完成 | 体检四项得分卡「一眼清楚」 |
| T5 | ✔ 完成 | — |
...
| T4 | ✔ 完成 | — |
| T5 | ✔ 完成 | 尝试暂需补充证据后追问：「待补数据清单提交后去哪再看？」当前只在提交时呈现一次（→ 问题 P-6） |
...
| P-4 | 置信度百分比诱发「不看反证直接采信」（自动化偏见） | 严重 | 1（B） | 工作台/复核 | ✖ 本期不改 |
| P-5 | 复核按钮无二次确认，担心误点 | 建议 | 1（A） | 复核 | ✖ 不改 |
| P-6 | 「暂需补充证据」产生的待补数据清单提交后无处回看 | 一般 | 1（C） | 复核 | ✖ 本期不改 |
...
- **P-4（置信度展示诱发偏见）：** 这正是任务 14「诊断复核」要对冲的核心问题——强制并排反证已是首层防线。是否进一步把百分比改为「高/中/低」三档，走查证据只有 1 人次，不足以推翻现设计；且该改动影响所有页面的信息密度。**列入文末未决问题 4，交由下期真人实测定夺。**
- **P-5（复核二次确认）：** 重新复核机制（异常 U-1）已兜底误操作；增加二次确认会提高每次复核的摩擦，降低校准反馈量（与设计原则 3 冲突）。保留单击复核 + 可重新复核。
...
设计目标只有一个：**让用户信任诊断结论**。所有动效服务于「确定性感」与「克制感」，不做装饰性动画。全部动效已在原型 v1.6 中用 CSS/JS 实现（实现位置见下表），时长 150–400ms（例外：模拟异步分析的加载态，以及证据定位闪烁 1.6s、新用例行背景闪淡 1.2s 两处驻留提示），不阻塞任何操作，并通过 `prefers-reduced-motion` 尊重系统减弱动效设置。
...
- **视觉方向「标准后台基线 + 诊断信息增强」（v1.6）：** 采用工程平台常见的浅灰背景、白色卡片、左侧导航和品牌蓝主操作；标题与正文统一使用无衬线字体，数据保留等宽字体。产品差异通过信息组织体现：首故障点清晰标记、证据引用可回跳、支持与反面证据并列。移除叙事性主题元素和衬线公文标题。
...
|---|---|---|---|---|---|
| R-1 | **诊断误报导致信任崩塌**：早期几次明显错误的「候选原因」会让团队永久放弃采信 | 产品核心价值失效 | 高 | TQ-01/TQ-03 持续跑批；线上「根因确认率」按周监控，跌破 40% 告警 | 规则先行（规则候选准确率天然高）；低置信降级为线索（G-2）；反面证据常驻，明示不确定性；发布节奏上先内部/设计伙伴使用 |
...
| R-5 | **复核反馈被污染**：随手乱点的复核、或单人偏见系统性地灌入校准数据，模型越校越歪 | 诊断质量长期劣化 | 中 | 校准数据集的复核人分布/耗时分布监控（<5s 的秒裁标记可疑）；TQ-03 按复核人分桶 | 「不成立」强制选理由（M5）；校准前按复核人加权与去噪；重新复核链保留（U-1）使污染可回溯清洗 |
| R-6 | **脱敏与快照的合规风险**：回归用例快照含未脱敏敏感数据并被回放/导出 | 合规事故 | 低–中 | TC-14 抽样审计；快照导出前二次扫描 | 采集端优先脱敏 + 服务端兜底（承接第一周 11.4 节）；快照存储加密、导出需复核人权限 |
...
- **信息层级：** 置信度作为次要信息，原始证据和差异对比优先；低置信度候选降级为线索，不进入正式复核流程。
...
1. 反面证据的生成质量未经真实数据验证——TQ-04 的 60% 有效率纯为目标值，需要用真实事故实测；
...
5. 选择「暂需补充证据」后生成的待补数据清单目前无持久化回看入口（走查 P-6），已记入 M5 的 P1 扩展需求；
6. 定价与计费未在本期展开（第一周 5.4 节建议按「有效 Agent Run + 诊断次数」计费，待商业化设计）。

**下期（第三周）验证项：**

1. 按第一周 11.6 节一周原型路线，用 10–15 条已知根因 Trace 实测诊断引擎 Top-3 命中率（目标 ≥ 70%）；
2. 找 3–5 名真实目标用户（SRE / Agent Owner）实测原型（本期为专家走查，见 10.5 节局限性）：复测三步闭环耗时、复核价值感知，并对置信度展示做「百分比 vs 三档」A/B 走查；
